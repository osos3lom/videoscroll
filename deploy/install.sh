...
#
#   sudo ./deploy/install.sh path/to/videoscroll   install or upgrade
#   sudo ./deploy/install.sh --rollback            go back to the previous binary
#
# The binary comes from `npm run build:server:linux` (dist-server/videoscroll)
# or from the "videoscroll-linux-amd64" GitHub Actions artifact.
#
...
BACKUP_ENV_FILE=/etc/videoscroll-backup.env
...
# ------------------------------------------------------------------ rollback --
if [[ "${1:-}" == "--rollback" ]]; then
    [[ -f "$APP_DIR/videoscroll.prev" ]] || die "no previous binary at $APP_DIR/videoscroll.prev"
    say "swapping $APP_DIR/videoscroll with the previous version"
    mv -f "$APP_DIR/videoscroll" "$APP_DIR/videoscroll.rollback"
    mv -f "$APP_DIR/videoscroll.prev" "$APP_DIR/videoscroll"
    # Keep the one we rolled back from, so a second --rollback undoes this.
    mv -f "$APP_DIR/videoscroll.rollback" "$APP_DIR/videoscroll.prev"
    systemctl restart videoscroll
    say "now running: $("$APP_DIR/videoscroll" version)"
    exit 0
fi
...
missing=()
for tool in ffmpeg ffprobe rsync; do
    command -v "$tool" >/dev/null || missing+=("$tool")
done
if ((${#missing[@]})); then
    say "installing ${missing[*]}"
    apt-get update -qq
    # ffprobe ships inside the ffmpeg package.
...
if [[ -f "$APP_DIR/videoscroll" ]]; then
    # Kept for `install.sh --rollback`.
    cp -p "$APP_DIR/videoscroll" "$APP_DIR/videoscroll.prev"
fi
install -m 755 "$BINARY" "$APP_DIR/videoscroll.new"
mv -f "$APP_DIR/videoscroll.new" "$APP_DIR/videoscroll"
install -m 755 "$REPO_DIR/deploy/backup.sh" "$APP_DIR/backup.sh"
...
if [[ ! -f "$BACKUP_ENV_FILE" ]]; then
    printf 'MEDIA_DIR=%s\nBACKUP_DIR=/mnt/videoscroll-backup\n' "$MEDIA_DIR" > "$BACKUP_ENV_FILE"
    chmod 644 "$BACKUP_ENV_FILE"
fi

say "systemd units"
for unit in videoscroll.service videoscroll-backup.service videoscroll-backup.timer; do
    install -m 644 "$REPO_DIR/deploy/$unit" "/etc/systemd/system/$unit"
done
systemctl daemon-reload
systemctl enable videoscroll >/dev/null
systemctl enable --now videoscroll-backup.timer >/dev/null
systemctl restart videoscroll
...
Installed $("$APP_DIR/videoscroll" version). First-time steps (see docs/self-hosting.md):
  ...
  4. Mount the USB backup disk at /mnt/videoscroll-backup (see the guide), then
       sudo systemctl start videoscroll-backup
  5. Set the GitHub repository variable VITE_API_ORIGIN to https://<your hostname>
...
Rollback:  sudo ./deploy/install.sh --rollback
EOF
